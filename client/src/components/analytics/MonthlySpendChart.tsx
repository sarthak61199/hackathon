...
export default function MonthlySpendChart({ data }: Props) {
  return (
    <Card className="p-5 analytics-card">
      <h3 className="text-sm font-semibold text-zinc-200 mb-4">Monthly Spend Trend</h3>
            ...
            content={({ active, payload, label }) => {
              ...
              return (
                <div className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs">
                  <p className="font-semibold text-zinc-100">{dayjs(label as string).format('MMMM YYYY')}</p>
                  <p className="text-zinc-400">{d.visits} visits</p>
                  <p className="text-indigo-300">{formatCompact(d.totalSpend)} total</p>
                  <p className="text-zinc-400">avg {formatCompact(d.avgSpend)}</p>
                </div>
              )
            }}
          />
